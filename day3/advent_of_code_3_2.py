...
first_wire_directions = first_line.split(',')
...
second_wire_directions = second_line.split(',')
...
#Now that we have the crosses lets just freaking brute force the least total steps each wire takes to get the the cross.
#This is something that defintely has a better solution than brute forcing but I don't want to spend the time to improve it.
    ...
    if direction[0] == 'L':
        for i in range(current_x, current_x + direction_amount):
            ...
            for i in range(0, len(crosses)):
                if current_x == crosses[i][0]:
                    if current_y == crosses[i][1]:
                        lengths[i] += current_distance
    if direction[0] == 'R':
        for i in range(current_x, current_x + direction_amount):
            current_x += 1
            current_distance += 1
